...
import torch
import datasets
from dpo.utils import TemporarilySeededRandom
...
def get_hh(split: str, silent: bool = False, cache_dir: str = None) -> Dict[
    ...
    """
    if os.path.exists(cache_dir + '/hh'):
        with open(cache_dir + f'/hh/hh_{split}.json', 'r', encoding='utf-8') as f:
            data = json.load(f)
        return data
    else:
...
def get_my_data(split: str, silent: bool = False, cache_dir: str = None, model_name: str = None,
                change_mode: str = None, dataset_name: str = None, mix_dataset: str = None,
                train_mode: str = None, data_type: str = 'choice'):
    dataset = {'data_path': f'data/{dataset_name}.csv',
               'result_path': f'result/{dataset_name}_result.csv'}
    if not os.path.exists(dataset['data_path']):
        print(
            f'{bcolors.FAIL}Cannot load train dataset, because we cannot handle dataset name {dataset_name}{bcolors.ENDC}')

    if model_name in default_model_prompt_template.keys():
        template = default_model_prompt_template[model_name]
        prompt_template, answer_template = template.split('{answer}')
        prompt_template = prompt_template.strip()
        answer_template = ('{answer}' + answer_template).strip()
    else:
        prompt_template = '{prompt}'

    result_path = dataset['result_path'].replace('.csv', f'_{model_name}_0_normal.csv')
    if os.path.exists(result_path):
        result_data = pd.read_csv(result_path)
        print(f'Read data from {result_path}')
    else:
        raise ValueError(f'No result path: {result_path}!')

    data = defaultdict(lambda: defaultdict(list))
    for row_idx, row in result_data.iterrows():
        idx = row_idx
        test_items = row[change_mode]
        lines = test_items.split('\n')

        if data_type == 'open':
            prompt = lines[0]
            prompt = prompt_template.format(prompt=prompt)
            model_answer = result_data.loc[idx, f'extract_{change_mode}_answer']
            if model_answer == 'A':
                chosen, rejected = lines[2], lines[1]
            else:
                chosen, rejected = lines[1], lines[2]
            chosen = chosen[2:].strip()
            rejected = rejected[2:].strip()
        else:
        ...
        if train_mode == 'sft':
            # For SFT mode, only include sft_target
            data[prompt]['sft_target'] = chosen
            data[prompt]['responses'] = [chosen]  # Only include chosen response
            data[prompt]['pairs'] = []  # No pairs needed for SFT
        else:
            responses = [chosen, rejected]
            n_responses = len(data[prompt]['responses'])
            data[prompt]['pairs'].append((n_responses, n_responses + 1))
    ...
    if mix_dataset == 'alpaca' and train_mode['name'] == 'sft':
        alpaca_data = read_file('data/reference/alpaca_data_cleaned.json')
        for i in range(len(data) * 9):
            prompt = f"{alpaca_data[i]['instruction']}\n{alpaca_data[i]['input']}"
            prompt = prompt_template.format(prompt=prompt)
            data[prompt]['pairs'] = []
            data[prompt]['responses'] = [alpaca_data[i]['output']]
            data[prompt]['sft_target'] = alpaca_data[i]['output']
    ...
    keys = list(data.keys())
    print(f'A data sample: \nPrompt: {keys[0]}, \nValues: {data[keys[0]]}')
    return data


def get_dataset(name: str, split: str, silent: bool = False, cache_dir: str = None, model_name: str = None,
                change_mode: str = None, train_mode: str = None):
    ...
    if name == 'shp':
        ...
        data = get_my_data(split, silent=silent, cache_dir=cache_dir,
                           model_name=model_name, change_mode=change_mode, dataset_name=name,
                           mix_dataset='alpaca', train_mode=train_mode)
...
def get_batch_iterator(names: List[str],
                       ...
                       train_mode: str = None) -> Iterator[Dict]:
    ...
    with TemporarilySeededRandom(seed):
        permutation_seeds = iter([int(x) for x in np.random.randint(0, 2 ** 32, size=1000000)])
        flat_data = []
        for name in names:
            truncation_mode = 'keep_end' if name == 'hh' else 'keep_start'
            for prompt, data in get_dataset(name, split, silent=silent, cache_dir=cache_dir, model_name=model_name,
                                            change_mode=change_mode, train_mode=train_mode).items():
